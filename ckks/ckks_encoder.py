...
class CKKSEncoder:
    ...
    def __init__(self, params):
        ...
        self.modulus = getattr(params, 'plaintext_modulus', None)  # If needed

    def encode(self, values, scaling_factor):
        """Encodes complex numbers into a polynomial.
        Encodes an array of complex numbers into a polynomial.
        Args:
        ...
        poly_enc = Polynomial(plain_len, message)
        # (optional) Modular reduction if modulus is provided
        if self.modulus:
            reducer = BarrettReducer(self.modulus)
            poly_enc = poly_enc.mod(self.modulus)
        return Plaintext(poly_enc, scaling_factor)
